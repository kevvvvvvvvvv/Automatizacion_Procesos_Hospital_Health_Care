...
const PacienteEdit: React.FC<EditProps> = ({ paciente }) => {
  const [form, setForm] = useState<Omit<Paciente, 'id'>>(initialState);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    router.put(`/pacientes/${paciente.id}`, form);
  };
      ...
      <form className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg text-black space-y-8" onSubmit={handleSubmit}>
        ...
        <fieldset className="border border-gray-600 rounded-md p-4">
            ...
            <InputField
                ...
                required
                maxLength={18}
                disabled // CURP suele ser inmutable, pero puedes remover si es necesario
            />
            <InputField
                id="nombre"
                label="Nombre"
        ...
        <div className="text-center space-x-4">
          <button
            type="button"
            onClick={() => router.get(`/pacientes`)} // Regresar al listado
